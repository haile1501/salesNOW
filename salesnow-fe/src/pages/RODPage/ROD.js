...
function Order() {
  ...
  const isReturnable = orderLines.some(
    (orderline) => orderline.quantity > orderline.returnQuantity
  );
  ...
  return (
    <div className={cx("order-detail-container")}>
              ...
              <div>
                ...
                <div className={cx("select-item")}>
                  ...
                  >
                    ...
                    <MenuItem value={PAYMENT_METHOD.TRANSFER}>
                      Chuyển khoản
                    </MenuItem>
                  </Select>
                </div>
      ...
      <div style={{ display: "flex", alignItems: "baseline", gap: "20px" }}>
        ...
        <Chip
          label={
            orderInfo.paymentStatus === PAYMENT_STATUS.INIT
              ? "Đang giao dịch"
              : "Hoàn thành"
          }
          variant="outlined"
      ...
      <div className={cx("infos")}>
        <div className={cx("customer-info", "panel")}>
          ...
          <Link
            to={`/customer/${orderInfo.customerId}`}
      ...
      <div className={cx("history")}>
        <div className={cx("historyHeader")}>
          <button
            onClick={handleHistoryPurchase}
            className={cx({ active: buttonHistoryPurchase })}
          >
            Thông tin sản phẩm
          </button>
          <button
            onClick={handleHistoryResponse}
            className={cx({ active: buttonHistoryResponse })}
          >
            Lịch sử trả hàng
          </button>
        </div>
          ...
          {buttonHistoryPurchase ? (
            <>
              <div className={cx("wrapItem")}>
                ...
                {orderLines.length === 0 && (
                  <div className={cx("historyEmpty")}>Không có dữ liệu</div>
                )}
              </div>
              <div className={cx("summary")}>
                ...
                <div style={{ fontWeight: 600 }} className={cx("summary-item")}>
                  <div>
                    {amount > returnAmount ? "Khách phải trả" : "Cần trả khách"}
                  </div>
                  <div
                    style={{
                      color:
                        amount > returnAmount ? "black" : "rgb(255, 77, 77)",
                    }}
                  >
                    {amount > returnAmount
                      ? numeral(amount - returnAmount).format(0, 0)
            ...
            <div className={cx("wrapItem")}>
              {returnHistories.map((item, index) => (
                <div
                  ...
                  key={item.returnOrderId}
                >
                  <Link
                    to={`/order_returns/${item.returnOrderId}`}
                    style={linkStyle}
                  >
                    <div
                      className={cx("returnOrderId")}
                  ...
                  <div className={cx("quantityReturn")}>
                    {item.returnQuantity}
                  </div>
                  <div className={cx("returnValue")}>
                    {numeral(item.returnValue).format("0,0")}
                  </div>
                  <div
                    className={cx("swapOrderId")}
                    style={{ color: "#0088FF", cursor: "pointer" }}
                  >
                    {item.swapOrderId ? (
                      <Link
                  ...
                  <div className={cx("swapValue")}>
                    {item.swapValue
                      ? numeral(item.swapValue).format("0,0")
                      : ""}
                  </div>
                </div>
              ))}
