package com.hust.salesnowbe.repositories.CustomerRepository;
...
//    @Query("SELECT new com.ict.salemanagement.dto.customer.CustomerSpendingDTO(o.customer, count(distinct o.order.id), sum(p.amount)) FROM Order o LEFT JOIN Payment p ON o.id = p.orderId GROUP BY o.customer ORDER BY o.customer.id DESC LIMIT :size OFFSET :offset", nativeQuery = true)
//    List<CustomerSpendingDTO> findAllCustomerBySpending(@Param("size") int size, @Param("offset") int offset);

//    @Query("SELECT new com.ict.salemanagement.dto.customer.CustomerSpendingDTO(SELECT o.customer_id, SUM(p.amount), COUNT(DISTINCT o.id)) " +
