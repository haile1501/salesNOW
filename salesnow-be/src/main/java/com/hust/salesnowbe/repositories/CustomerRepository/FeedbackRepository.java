...
public interface FeedbackRepository extends JpaRepository<Feedback, Integer> {
    ...
    @Query(value = "SELECT * FROM feedback f WHERE f.customer_id = :customerId ORDER BY f.id DESC", nativeQuery = true)
    List<Feedback> findAllFeedbackByCustomer(@Param("customerId") int customerId);

    @Query(value = "SELECT * FROM feedback f LIMIT :size OFFSET :offset", nativeQuery = true)
