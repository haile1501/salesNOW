...
public class CustomerServices {
    ...
    public List<CustomerSpendingDTO> getAllCustomerBySpending(int page, int size) {
        ...
        for (Object[] data : customerDataList) {
            ...
            BigDecimal totalAmount = (data[1] != null) ? BigDecimal.valueOf(((Number) data[1]).doubleValue())
                    : BigDecimal.ZERO;
            BigDecimal orderCount = (data[2] != null) ? BigDecimal.valueOf(((Number) data[2]).doubleValue())
                    : BigDecimal.ZERO;
    ...
    public void deleteCustomerById(int customerId) {
        ...
        if (exists) {
            ...
            // Nếu không tìm thấy khách hàng, bạn có thể ném một ngoại lệ hoặc xử lý theo ý
            // muốn
